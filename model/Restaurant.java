...
public class Restaurant {
	...
	public Restaurant(JSONObject object) { //Constructor : JSON data => Java data type
	   	 try { 
	   		 if (object != null) {
	   			 this.businessId = object.getString("id");
	   			 JSONArray jsonArray = (JSONArray) object.get("categories");
	   			 List<String> list = new ArrayList<>();
	   			 for (int i = 0; i < jsonArray.length(); i++) { //nested JSONArray
	   				 JSONArray subArray = jsonArray.getJSONArray(i);
	   				 for (int j = 0; j < subArray.length(); j++) {
	   					 list.add(parseString(subArray.getString(j)));
	   				 }
	   			 }
	   			 //list = Chinese, Japanese => "Chinese, Japanese"
